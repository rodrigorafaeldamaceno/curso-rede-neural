#!/usr/bin/env python3
import numpy as np
# exercício: fase forward na forma de tensores
...
class Xor:
  ...
  def __init__(self):
    ...
    ocultos = 2
    saidas = len(self.d[0])
    self.w1 = np.random.randn(ocultos, entradas) / 2  # pesos da camada oculta
    self.b1 = np.zeros(ocultos)                       # biases da camada oculta
    self.w2 = np.random.randn(saidas, ocultos) / 2    # pesos da camada de saída
