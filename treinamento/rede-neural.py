...
class RNA:
    ...
    def __init__(self):
        self.w = np.array([             # arrays numpy são mais eficitentes
            [0., 0.]                    # criando array de float no numpy
        ])
        # convenção: bias é vetor coluna (aprofundamento abaixo)
        self.b = np.array([0.])
        self.dataset = np.array([       # conveniência: dataset armazenado na classe RNA (em redes reais isso não ocorre)
            # convenção: instância é vetor coluna (aprofundamento abaixo)
            [0, 0],
            [0, 1],
            [1, 0],
            [1, 1]
        ...
        # saídas desejadas para o and logico
    ...
    def ativaDataset(self):
        ...
        for i in range(10):
            for x in range(len(self.dataset)):
                self.ativaInstancia(self.dataset[x], self.d[x])
            print("Epoca ", i+2)

        print(self.w, self.b)

    def treinamento(self, x, y, d):

        self.w -= self.taxa * x * (y-d)
        self.b -= self.taxa * (y-d)
        #y = self.f(x)

    def ativaInstancia(self, x, d):
        # for i in range(10):
        y = self.f(x)
        if(d != y):
            self.treinamento(x, y, d)
    ...
    def passo(self, v):
        # y tem as mesmas dimensões de v e é preenchido com zeros
        y = np.zeros(v.shape)
